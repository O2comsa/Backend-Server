                ...
                <div class="block">
                    ...
                    <div class="block-content tab-content">
                        <div class="tab-pane fade show active px-2" id="details" role="tabpanel" aria-labelledby="nav-home-tab">
                            {!! Form::open(['route' => ['users.update.details', $user->id], 'method' => 'PUT', 'id' => 'details-form','files' => true,]) !!}
                            @include('Admin.users.partials.details', ['profile' => false])
                            {!! Form::close() !!}
                        </div>

                        <div class="tab-pane fade px-2" id="login-details" role="tabpanel" aria-labelledby="nav-profile-tab">
                            {!! Form::open(['route' => ['users.update.login-details', $user->id], 'method' => 'PUT', 'id' => 'login-details-form']) !!}
                            @include('Admin.users.partials.auth')
                            {!! Form::close() !!}
                        </div>
                    </div>
                </div>
